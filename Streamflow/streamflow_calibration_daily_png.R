...
file_index <- 53
sim_name <- "WY2006-2015" #simulation water years of the configuration file
startyear <- "2005" #start year of data that will be analyzed
endyear <- "2015" #end year of data that will be analyzed
basin <- "SF" #uppernook, NF, MF, SF
...
for (sim_number in file_index) { #loops through each file index; allows stats and figures for multiple sims to be created
  ...
  sim_folder <- paste0(basin, "_", sim_name, "_", sim_number) #DHSVM output directory w/ indexing value; same name as the configuration file
  ...
  for (gauge in gauges) {
    ...
    #name of image file
    plot_name <- paste0("Streamflow_daily",  gauge, " ", startyear, "-", endyear, "_", sim_number, ".png") #used in the naming of the hist_flow file
    ...
    #convert from m^3 per timestep (as ts which is set above) to cms
    ...
    if (gauge == "CEDARVILLE") {
      ...
      #yscale <- scale_y_continuous(name = ylabel, limits = c(0, 1400), breaks = seq(0, 1200, 300)) #sets a constant y-axis limit for a consistent visual comparison
      gauge <- 'North Cedarville'
    } else if (gauge == "NFUSGS") {
      hist_flow <- as.data.table(read.table("NFUSGS_WY1988-2020.csv", sep=",", header=F))
      #yscale <- scale_y_continuous(name = ylabel, limits = c(0, 300), breaks = seq(0, 280, 70)) #300
      gauge <- 'North Fork'
    } else if (gauge == "MFUSGS") {
      hist_flow <- as.data.table(read.table("MFUSGS_WY1996-2020.csv", sep=",", header=F))
      #yscale <- scale_y_continuous(name = ylabel, limits = c(0, 250), breaks = seq(0, 240, 60)) #250
      gauge <- 'Middle Fork'
    } else if (gauge == "SFUSGS") {
      hist_flow <- as.data.table(read.table("SFUSGS_WY2009-2020.csv", sep=",", header=F))
      #yscale <- scale_y_continuous(name = ylabel, limits = c(0, 400), breaks = seq(0, 360, 90)) #400
      gauge <- 'South Fork'
    } else if (gauge == "WICKERSHAM") {
      hist_flow <- as.data.table(read.table("wickersham_WY2000-2008.csv", sep=",", header=F))
      #yscale <- scale_y_continuous(name = ylabel, limits = c(0, 360), breaks = seq(0, 320, 80))
    }
    ...
    yscale <- scale_y_continuous(name = ylabel, limits = c(0, (max(flow_dailyclean$flow))))
    
    #sets the x-label breaks of your plot
    #xscale <- scale_x_date(name = NULL, date_breaks = "1 month", date_labels = "%b") #breaks every 1 month (good for 1 year simulation)
    xscale <- scale_x_date(name = NULL, date_breaks = "1 year", date_labels = "%Y") #breaks every 1 year (good for longer simulations)
